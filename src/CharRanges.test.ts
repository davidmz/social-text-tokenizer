...
describe('CharRanges', () => {
  ...
  describe('add', () => {
    it('should add range', () => {
      cr.add([1, 2]);
      expect(cr.toTuples()).toEqual([[1, 2]]);
    });

    it('should add range without explicit end', () => {
      cr.add(1);
      expect(cr.toTuples()).toEqual([[1, 1]]);
    });
    ...
    it('should not add range contained in existing ranges', () => {
      ...
      expect(cr.toTuples()).toEqual([[1, 10]]);
    });

    it('should extend existing ranges', () => {
      cr.add([1, 10], [2, 15]);
      expect(cr.toTuples()).toEqual([[1, 15]]);
    });
    ...
    it('should add non-overlaping range', () => {
      cr.add([1, 5]);
      cr.add([7, 10]);
      expect(cr.toTuples()).toEqual([[1, 5], [7, 10]]);
    });
  ...
  describe('remove', () => {
    it('should not remove range if it outside of existing ranges', () => {
      ...
      cr.add([6, 7]);
      cr.remove([8, 10]);
      expect(cr.toTuples()).toEqual([[1, 7]]);
    });
    ...
    it('should remove existing range if it is overlapped by range to remove', () => {
      ...
      cr.add([7, 9]);
      cr.remove([6, 9]);
      expect(cr.toTuples()).toEqual([[1, 5]]);
    });
    ...
    it('should shrink existing range at left if it is partially overlapped by range to remove', () => {
      cr.add([1, 5]);
      cr.remove([1, 3]);
      expect(cr.toTuples()).toEqual([[4, 5]]);
    });

    it('should split existing range if removed area is inside it', () => {
      cr.add([1, 5]);
      cr.remove([2, 3]);
      expect(cr.toTuples()).toEqual([[1, 1], [4, 5]]);
    });
  });
